...
def main():
    ...
    for repo_key in sorted(repo_stats.keys()):
        ...
        print(
            f"  Repo {repo_key}: {total} backups ({full_count} full, {incr_count} incr, {diff_count} diff)"
        )
    print()
    ...
    print("─" * 60)
    print(
        f"  {'Repo':<6} {'Type':<6} {'Timestamp':<18} {'Age':<10} {'DB Size':<10} {'Backup Δ':<10}"
    )
    ...
    for repo_key in sorted(repo_stats.keys()):
        ...
        print(
            f"  {repo_key:<6} {backup_type:<6} {format_timestamp(ts):<18} {time_ago(ts):<10} {format_size(db_size):<10} {format_size(delta_size):<10}"
        )
    print()
    ...
    for repo_key in sorted(repo_stats.keys()):
        ...
        latest_full = max(
            full_backups, key=lambda x: x.get("timestamp", {}).get("stop", 0)
        )
        ...
        print(
            f"  {repo_key:<6} {label:<26} {format_timestamp(ts):<18} {format_size(size):<12}"
        )
    print()
    ...
    for repo_key in sorted(repo_stats.keys()):
        ...
        sorted_incr = sorted(
            incr_backups,
            key=lambda x: x.get("timestamp", {}).get("stop", 0),
            reverse=True,
        )[:5]
        ...
        for backup in sorted_incr:
            ...
            duration = backup.get("timestamp", {}).get("stop", 0) - backup.get(
                "timestamp", {}
            ).get("start", 0)

            print(
                f"    {format_timestamp(ts)} | Δ {format_size(delta):>8} → {format_size(repo_delta):>8} (compressed) | {format_duration(duration):>6}"
            )
        print()
    ...
    all_backups_sorted = sorted(
        backups, key=lambda x: x.get("timestamp", {}).get("stop", 0)
    )
    ...
    for backup in backups:
        ...
        if backup.get("type") == "full":
            total_repo_size[repo_key] = max(
                total_repo_size[repo_key], repo_info.get("size", 0)
            )

    for repo_key in sorted(total_repo_size.keys()):
        print(
            f"  Repo {repo_key}: ~{format_size(total_repo_size[repo_key])} (full backup size)"
        )

    print()
    print("─" * 60)
    print(
        f"  Total backups: {len(backups)} | Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
    print()
