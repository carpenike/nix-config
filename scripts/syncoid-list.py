...
def get_syncoid_metrics_from_prometheus(
...
def get_status_display(
    status: Optional[int], last_success: Optional[float]
) -> tuple[str, str, str]:
    """
...
def warmup_ssh_connection(host: str, console: Console, quiet: bool = False) -> bool:
    ...
    try:
        result = subprocess.run(
            ["ssh", host, "echo ok"], capture_output=True, text=True, timeout=60
        )
...
def discover_syncoid_services(host: str) -> List[Dict[str, Any]]:
    ...
    cmd = (
        "systemctl list-units --type=timer --all 'syncoid-*.timer' --no-legend --plain"
    )
...
def print_table_output(
    jobs: List[Dict[str, Any]], console: Console, source: str = "prometheus"
):
    ...
    for job in jobs_sorted:
        ...
        status_text, color, details = get_status_display(
            job.get("status"), job.get("last_success")
        )
        ...
        table.add_row(
            dataset_short, target_display, status_colored, last_success_str, details
        )
    ...
    console.print()
    console.print(
        f"[dim]Total: {total} | OK: {stats['ok']} | Running: {stats['running']} | Stale: {stats['stale']} | Failed: {stats['failed']}[/dim]"
    )
    console.print(
        f"[dim]Data source: {source} | Stale threshold: {STALE_THRESHOLD_HOURS} hours[/dim]"
    )
...
Examples:
    ...
    parser.add_argument("--dataset", help="Filter by dataset name (partial match)")
    parser.add_argument("--target", help="Filter by target host (partial match)")
    parser.add_argument(
        "--verify",
        action="store_true",
    ...
    if args.verify:
        ...
        if not args.api_key:
            console.print(
                "[yellow]Warning: PROMETHEUS_API_KEY not set. Some endpoints may require authentication.[/yellow]"
            )
    ...
    if args.target:
        jobs = [
            j for j in jobs if args.target.lower() in j.get("target_host", "").lower()
        ]
