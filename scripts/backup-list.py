...
try:
    ...
    print(
        "Error: 'rich' library not found. Install it with: pip install rich",
        file=sys.stderr,
    )
    sys.exit(1)
...
def query_prometheus(
    base_url: str, query: str, api_key: Optional[str] = None
    ...
    try:
        response = requests.get(
            url, params={"query": query}, headers=headers, timeout=30, verify=False
        )
...
def get_backup_metrics_from_prometheus(
...
def verify_snapshots(
    host: str, job_name: str, env: Dict[str, str], limit: int
) -> Optional[Dict[str, Any]]:
...
""",
    ...
    parser.add_argument(
        "--service", "-s", default=None, help="Filter by service name (partial match)"
    )
    parser.add_argument("--repo", "-r", default=None, help="Filter by repository name")
    parser.add_argument(
        "--verify",
        "-v",
        action="store_true",
    ...
    if args.verify:
        ...
        if not args.quiet and not args.json_output:
            console.print(
                f"[yellow]Verify mode:[/yellow] Querying restic directly on {full_host}..."
            )
            console.print("[dim]This may take a while...[/dim]")

        # Warm up SSH connection to trigger FIDO key PIN prompt
        if not warmup_ssh_connection(
            full_host, console, quiet=args.quiet or args.json_output
        ):
            if args.json_output:
                print(
                    json.dumps(
        ...
        if not services:
            if args.json_output:
                print(
                    json.dumps(
                        {
                            "error": "No Restic backup services found",
                            "services": [],
                            "mode": "verify",
                        }
                    )
                ...
                console.print(
                    f"[yellow]No Restic backup services found on {full_host}[/yellow]"
                )
            return 1
        ...
        if args.json_output:
            output = {
                ...
                "summary": {
                    ...
                    "total_snapshots": sum(
                        len(r["snapshots"])
                        for r in results
                        if isinstance(r["snapshots"], list)
                    ),
                },
            }
            ...
            if not args.quiet:
                ...
                console.print(
                    f"  Services verified: [green]{len([r for r in results if r.get('verified')])}[/green] / {len(results)}"
                )
        ...
        if not args.quiet and not args.json_output:
            console.print(
                f"[dim]Querying Prometheus metrics from {prometheus_url}...[/dim]"
            )
        ...
        if not metrics:
            if args.json_output:
                print(
                    json.dumps(
                        {
                            "error": "No backup metrics found in Prometheus",
                            "services": [],
                            "mode": "prometheus",
                        }
                    )
        ...
        if args.service:
            metrics = [
                m for m in metrics if args.service.lower() in m["job_name"].lower()
            ]
        ...
        if args.repo:
            metrics = [
                m
                for m in metrics
                if args.repo.lower() in m.get("repository_name", "").lower()
            ]
        ...
        if args.json_output:
            ...
            if not args.quiet:
                console.print()
                console.print(
                    "[dim]Tip: Use --verify to validate data directly from restic repositories[/dim]"
                )
