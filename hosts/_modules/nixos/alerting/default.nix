...
let
  inherit (lib) mkOption types mkIf length filterAttrs mapAttrsToList generators mkAfter recursiveUpdate attrsToList;
  ...
  amPost = pkgs.writeShellScriptBin "am-postalert" ''
    ...
    # Build payload once; include unit label only if it was provided
    payload="$(cat <<'EOF'
[
  {
    "labels": {
...
receivers:
  ...
  # Collect PromQL rules into Prometheus alert groups
  promqlRules = filterAttrs (_: r: r.type == "promql") cfg.rules;
  toPromRule = name: r: {
    alert = r.alertname;
    expr = r.expr;
    for = r.for;
    labels = r.labels // {
      severity = r.severity;
      service = r.labels.service or name;
    };
    annotations = {
      summary = r.annotations.title or "${name} alert";
      description = r.annotations.body or "";
    };
  };
  promGroups = [{
    name = "homelab-alerts";
    rules = mapAttrsToList toPromRule promqlRules;
  }];
  alertsYml = pkgs.writeText "alerts.yml" (generators.toYAML {} { groups = promGroups; });

  # Event rule units: for each event rule create alert-<key>@.service
  eventRules = filterAttrs (_: r: r.type == "event") cfg.rules;
  mkEventUnit = name: r: {
    description = "Post Alertmanager event for ${name}";
    serviceConfig = {
      Type = "oneshot";
      ExecStart = ''
        ${amPost}/bin/am-postalert \
          ...
          ${r.alertname} \
          ${r.severity} \
          ${r.labels.service or name} \
          "${r.annotations.title or "${name} event"}" \
          "${r.annotations.body or ""}" \
          %I
      '';
    };
  };
  eventUnits = lib.mapAttrs' (name: r: { name = "alert-${name}@"; value = mkEventUnit name r; }) eventRules;

  # OnFailure attachment to target units using mkAfter list merge
  onFailureAttach =
    lib.foldl'
      (acc: pair:
        let
          name = pair.name;
          r = pair.value;
          unit = r.systemd.unit or null;
        in
          if r.systemd.onFailure.enable && unit != null then
            recursiveUpdate acc {
              systemd.services.${unit}.serviceConfig.OnFailure = mkAfter [ "alert-${name}@%n.service" ];
            }
          else acc
      )
      {}
      (attrsToList eventRules);
...
in
{
  options.modules.alerting = {
    ...
    export.ruleFiles = mkOption {
      type = types.listOf types.path;
      default = [];
      readOnly = true;
      description = "Aggregated Prometheus alert rule files generated by the alerting module. Consumed by monitoring.nix.";
    };
  };
  ...
  config = mkIf cfg.enable (
    {
      ...
      # SOPS secrets for Alertmanager Pushover receiver; readable by alertmanager user
      sops.secrets.${cfg.receivers.pushover.tokenSecret} = {
        owner = "alertmanager"; group = "alertmanager";
      };
      sops.secrets.${cfg.receivers.pushover.userSecret} = {
        owner = "alertmanager"; group = "alertmanager";
      };
      ...
      services.prometheus.alertmanager.extraFlags = [ "--config.file=/etc/alertmanager/alertmanager.yml" ];
      systemd.services.prometheus-alertmanager = {
        wants = [ "alertmanager-config.service" ];
        after = [ "alertmanager-config.service" ];
      };

      # Export aggregated Prometheus alert rules (consumed by monitoring.nix)
      modules.alerting.export.ruleFiles = lib.optional ((length (builtins.attrNames promqlRules)) > 0) alertsYml;

      # Install helper script
      environment.systemPackages = [ amPost ];

      # Attach OnFailure to target units for event rules
    }
    // onFailureAttach
    //
    {
      # Event alert templated units (one per rule) + Boot/shutdown system events
      systemd.services = eventUnits // {
        alert-boot = {
