  ...
  mkAutheliaProtectedService = {
    ...
    mkIf condition {
      modules.reverseProxy.virtualHosts.${name} = {
        ...
        # No basic auth - using forward auth instead
        auth = null;
        ...
        vendorExtensions.caddy.extraConfig = ''
          ...
          forward_auth ${autheliaUrl} {
            uri /api/verify?rd=https://auth.''${config.modules.reverseProxy.domain}
            copy_headers Remote-User Remote-Groups Remote-Name Remote-Email
          }
        '';
      };

      # Add access control rule to Authelia
      # Note: This requires the Authelia module to aggregate rules from all services
      modules.services.authelia.accessControl.rules = mkBefore [{
        domain = if domain != null then "${subdomain}.${domain}" else "${subdomain}.\${config.modules.reverseProxy.domain}";
        policy = if require2FA then "two_factor" else "one_factor";
        subject = map (g: "group:${g}") allowedGroups;
      }];
    };
}
