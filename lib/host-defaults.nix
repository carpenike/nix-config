...
in
{
  ...
  # Container counterpart to mkSystemdServiceDownAlert. Also emits a paired
  # <displayName>RestartChurn rule -- see that helper for the reasoning.
  #
  # container_service_active is not a recording rule: it comes from the
  # textfile collector in hosts/forge/infrastructure/containerization.nix,
  # which shells out to `systemctl is-active podman-<name>.service` on a 60s
  # timer. That makes it blind to crash-loops for the same reason the systemd
  # state gauge is -- a container that reaches active before dying reports 1 --
  # and at a coarser sampling interval, so the counter-based companion matters
  # here at least as much.
  #
  # Note the name asymmetry: the exporter strips the `podman-` prefix, so the
  # metric label is bare `${serviceName}` while the unit is
  # `podman-${serviceName}.service`. The churn rule queries systemd directly
  # and therefore needs the prefix restored -- without it the generated rule
  # would reference a nonexistent unit and silently never fire.
  mkServiceDownAlert = serviceName: displayName: description: {
    ...
    restartChurn = {
      unit = "podman-${serviceName}.service";
  ...
  # Emits TWO alerts: the state-based ServiceDown rule below, and a companion
  # <displayName>RestartChurn generated by modules/nixos/alerting/default.nix
  # from the restartChurn block.
  #
  # The pairing is deliberate. The ServiceDown expression only fires when the
  # unit sits inactive for a contiguous 2 minutes, which a crash-looping unit
  # never does -- it is scraped as `active` between restarts, and a unit with
  # StartLimitIntervalSec=0 can never park in `failed` at all. On 2026-08-16
  # that gap let Home Assistant restart 367 times in 43 minutes with this rule
  # stuck in `pending`. The churn alert counts restarts instead, so a loop is
  # caught whether or not the unit ever comes to rest.
  #
  # serviceName must be the real unit name minus the .service suffix. Four
  # call sites were previously pointed at units that do not exist (beszel,
  # cloudflared, emqx, searx), which produced rules that could never match a
  # series; verify against `systemctl list-units` when adding one.
