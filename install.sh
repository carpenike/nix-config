...
nix-shell -p git --command "git clone https://github.com/carpenike/nixos-config $SCRIPT_DIR"
...
sed -i "0,/ryan/s//$(whoami)/" $SCRIPT_DIR/flake.nix
sed -i "0,/Ryan/s//$(getent passwd $(whoami) | cut -d ':' -f 5 | cut -d ',' -f 1)/" $SCRIPT_DIR/flake.nix
sed -i "s/ryan@ryanholt.net//" $SCRIPT_DIR/flake.nix
