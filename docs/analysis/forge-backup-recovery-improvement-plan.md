...
- Added a fail-closed Music Assistant preseed prototype using the shared storage helper, a hard systemd dependency, and explicit `allowEmptyBootstrap = false`. Adopted its verified existing dataset with the ZFS completion property, confirmed the deployed preseed skipped safely, and reached 26 classified datasets with no Music Assistant coverage gaps.
- Observed a safe guarded-deployment refusal when Sanoid completed just after its start job; all 115 timers were restored with no failed units, and a retry from the verified idle window activated cleanly.
- Generalized policy-driven fail-closed preseed to Bichon, Home Assistant, PocketID, Zigbee2MQTT, and Z-Wave JS UI. Each main unit now requires successful preseed when its dataset policy forbids empty bootstrap; all five deployed preseed units skipped on verified completion markers and restarted healthy.
- Extended policy-driven fail-closed preseed to all nine standard datasets. The shared service factory now specializes from dataset policy while unclassified services remain compatibility-soft; direct wrappers cover Apprise, ESPHome, Music Assistant, and NetVisor.
- Repaired ESPHome's dependency override to target the real OCI unit and added NetVisor's previously missing main-service preseed dependency. All 14 classified non-empty application preseed paths now require success, and all restarted services passed their configured healthchecks after deployment.
- Implemented Actual's advertised but previously missing preseed service. Both finance databases passed SQLite integrity checks before marker adoption and after activation; Actual now requires fail-closed preseed success and reports only `offsite-backup` as missing.
- Classified Omada's embedded MongoDB controller state as critical, enabled inherited Syncoid recovery, and enforced fail-closed preseed startup. The deployed controller retained its WiredTiger catalog and journals and reports only `offsite-backup` as missing.
- Deferred Paperless classification until its split recovery boundary is represented: original documents live on NAS, metadata lives in PostgreSQL, and the local ZFS dataset contains application state, search indexes, and thumbnails.
- Fixed Restic interruption handling after Scrypted exposed a production-only false-green metric: systemd had terminated the wrapper and Restic together before EXIT cleanup ran. Wrappers now use mixed kill mode, track and stop the Restic child, and pass an isolated real `systemctl stop` regression.
- Removed Scrypted's stale pre-fix success metric under guarded checks. Its wrapper passes all mocked result modes, but classification remains deferred until the next natural backup recreates current complete telemetry.
- Audited all 52 Restic jobs and removed 21 additional legacy metrics only where the last process was killed by TERM (`ExecMainCode=2`, status 15). Valid status-0 legacy successes were retained; the resulting state is 11 current-schema, 19 valid legacy, and 22 intentionally absent metrics.
- Natural metric refresh begins at 00:03 EDT on July 14; Grocy and Scrypted remain unclassified pending their scheduled 00:31 and 00:34 complete runs rather than forced high-load backups.
- Classified Grafana OnCall's SQLite escalation and schedule state as standard and enabled fail-closed preseed. Replaced a stale nonexistent preseed reference with the enforced chain `preseed -> Redis -> migration -> engine -> Celery`; the deployed database passed SQLite integrity and the stack is healthy.
- Classified Cooklang's 332-recipe source collection as standard, enabled policy-driven fail-closed preseed, and verified its production Git sync converges cleanly with the remote repository. Classified Cooklang Federation's database and search index as ephemeral derived state rebuilt from declarative Git feeds.
- Classified Homepage, Recyclarr, Kometa, and WorldMonitor as ephemeral rebuildable state. Their convenience snapshots, replication, and Restic jobs may remain, but policy now permits empty bootstrap because Nix configuration and upstream sources recreate their data.
...
- The first complete overnight metric refresh reported 45 complete and seven partial Restic jobs. Enclosed, Prowlarr, Qui, SearXNG, Sonarr, The Lounge, and Tracearr all returned exit 3 because the unprivileged backup user could not read their live paths.
- Removed Restic, preseed, and Syncoid contributions for disposable Enclosed, SearXNG, and Tracearr state. Enclosed is now explicitly ephemeral because restoring retained encrypted notes would violate TTL and self-destruct semantics; Tracearr's durable records remain protected through centralized PostgreSQL and pgBackRest.
- Converted Prowlarr, Qui, Sonarr, and The Lounge to read-only ZFS clone backups with unit-scoped `CAP_DAC_READ_SEARCH`. Classified Qui's SQLite state and The Lounge's accounts, sessions, and history as standard with 24-hour onsite RPO, 8-hour RTO, complete NAS protection, and fail-closed preseed startup.
- Completed The Lounge's previously missing preseed implementation and repaired Qui's override to target the real Podman unit. Activation exposed and removed Qui's stale dependency on nonexistent `sops-nix.service`; its required SOPS environment file remains fail-closed.
- Deployed the 45-dataset manifest (9 critical, 14 standard, 20 ephemeral, 2 system). Forge now has 109 valid backup timers; removed jobs and stale metrics are absent, and Prometheus reports only the expected Watchdog alert.
- `task backup:validate-restic -- --run` passed all result, clone, snapshot, lock, and timer checks for Prowlarr, Qui, Sonarr, and The Lounge. Restic telemetry now reports 49 complete jobs with no partial or failed results.
- Generic restores preserved size, mode, UID, and GID for all four repaired jobs. Semantic restores validated Qui snapshot `8c31f169` (`integrity=ok`, 51 tables) and The Lounge snapshot `4d251230` (`integrity=ok`, 5 tables, valid user JSON).
