...
**Last Reviewed**: 2026-08-29 (hermes-agent entry only; full review still due)
...
### Hermes Agent - Signal, MCP Recovery, Cold-Store, and OAuth Fixes

| Field | Value |
| --- | --- |
| **Added** | 2026-07-27 (rebased onto upstream v0.20.2 on 2026-08-16) |
| **Location** | `flake.nix` (`inputs.hermes-agent`), `flake.lock`, and `hosts/forge/services/hermes-agent.nix` |
| **Affects** | Forge Signal gateway, Hermes finance MCP tools after boots/deployments, Forge evaluation, CI flake checks, automated input updates, and fixed-port MCP OAuth reauthorization |
| **Reason** | The released Signal adapter targets direct signal-cli SSE/JSON-RPC endpoints that do not exist in signal-cli-rest-api 0.100; upstream's `importNpmLock { npmRoot = fileset.toSource ...; }` breaks cold-store evaluation; and the OAuth paste fallback closes a socket while its blocking `handle_request()` thread still owns the fixed callback port, causing the SDK's next callback to fail with `Address already in use`. Verified 2026-08-17: upstream's `allow_reuse_address` fix (#44590) does NOT cover the OAuth symptom on Linux — on pure v0.20.2 a second fixed-port flow after a pasted callback fails with `[Errno 98] Address already in use` (the `handle_request()` thread keeps the closed listener's socket alive; `SO_REUSEADDR`/`SO_REUSEPORT` cannot help). macOS masks the bug, which likely explains why upstream has not hit it. The fork's managed-shutdown fix cures it (probe tests pass on the fork branch on the same host). Verified 2026-08-26: if the long-lived gateway starts while `homelab-mcp` is unavailable, both OAuth HTTP aliases can remain cancelled across later turns even though a fresh `hermes mcp test` connects immediately; restarting the gateway recovers them. The standalone Signal fallback used by manual `hermes cron run` also cannot derive a REST recipient by prefixing the 44-character raw JSON-RPC group `internal_id`: `/v2/send` requires the separate 66-character canonical `id` returned by `/v1/groups`, so that path fails with `Invalid identifier`; normal gateway-scheduled JSON-RPC delivery is unaffected. |
| **Workaround** | Pin `hermes-agent` to immutable fork commit [`9dfd2fc32`](https://github.com/carpenike/hermes-agent/commit/9dfd2fc325f60d24d349ad47a5cf571899cede3c) (branch `nix-config/v2026.8.16`): upstream **v0.20.2** + the current PR #53696 head (REST/WebSocket adapter) + initial v0.100 group-recipient handling + the PR #72689 cold-store fix ported to the refactored `hermesNpmLib` (only `npmRoot = repoRoot` remains needed; desktop/tui/web eval-time reads were removed upstream) + managed OAuth listener shutdown. Forge also orders Hermes after `homelab-mcp.service` and waits for its local `/healthz` endpoint before launching the long-lived gateway, while keeping the dependency optional so later MCP restarts do not tear down Signal or Telegram. Do not use manual production `hermes cron run` as a Signal recovery path until its REST fallback resolves `internal_id` through `/v1/groups`; use the local-only dry-run units for acceptance and let the live gateway scheduler own production delivery. **Added 2026-08-29: `hermes-agent-mcp-recover`** (`hosts/forge/services/hermes-agent.nix`) — a 10-minute timer that greps the journal for `failed initial connection after 3 attempts, parking until a reconnect is requested` and restarts `hermes-agent`, since restart is the only recovery from the poisoned lock and is complete (`hermes mcp test holthome` went from `TimeoutError` to `Connected (561ms), 13 tools`). Guarded by a 900s minimum uptime (so a just-started gateway is left to settle, bounding it to one restart per interval) and a 3-restarts-per-6h circuit breaker that stops, logs loudly and exits non-zero if restarting is not working — on the grounds that a restart loop would mask a different fault and churn Signal/Telegram. Deliberately a separate timer rather than the heartbeat's `OnFailure`: the heartbeat sweeps 6-hourly, and this leaves the working detect/page chain untouched. **Remove it when the fork rebases onto a release containing `9a1eef7a`** (see Check), after confirming a long-lived gateway survives a homelab-mcp restart. |
| **Validation** | 2026-08-16 on the rebased tree: Signal suites + full OAuth file pass (`144 passed, 1 skipped`); merge-adjacent suites (`test_status`, `test_send_message_tool`, `test_signal_media`, `test_mcp_reconnect_signal`) pass (`116 passed, 5 skipped`); `nix eval .#packages.x86_64-linux.default.name` → `hermes-agent-0.20.2`. All service-module assumptions re-verified on 0.20.2: NixOS module options, cron CLI flags/output format (`Name:`/`[paused]`/`[active]`, 12-hex IDs), `cron.jobs.get_job/update_job` + `context_from`, `HERMES_PYTHON` shell-wrapper export, `platform_toolsets`, `channel_overrides` in the cached-agent signature, oauth `client_secret`/`redirect_port` config keys, `[SILENT]` filter, `cron.wrap_response`, `errors.log` path, `## Response`/`## Error` output markers. On 2026-08-26, a Forge restart recovered both MCP aliases (21 registered tools), the new startup gate deployed successfully, and Signal API confirmed raw `internal_id` and REST `id` are distinct identifiers. Validate nix-config with `task nix:build-nixos host=forge`, then verify Signal reply and scoped MCP login on forge. |
| **Check** | After upstream PRs #53696, #72689, and #88131 all merge, restore `url = "github:NousResearch/hermes-agent"`, update only the Hermes lock node, and rerun the cold-store, Signal, and fixed-port OAuth checks before removing this entry. The OAuth fork commit is confirmed still required on Linux (2026-08-17 probe on forge); #88131 upstreams it. Re-evaluate the homelab-mcp startup health gate after #77765 is fixed and a long-lived gateway demonstrably recovers from an unavailable initial OAuth HTTP MCP connection. Before declaring standalone Signal fallback fixed, manually verify that it maps `/v1/groups[].internal_id` to the matching canonical `.id` instead of constructing `group.${internal_id}`. **NEW REBASE TRIGGER (2026-08-29): upstream has FIXED the reconnect wedge, but it is unreleased.** PR [#97458](https://github.com/NousResearch/hermes-agent/pull/97458) (`9a1eef7a`, merged 2026-08-29 03:16) replaces the SDK's task-owned `anyio.Lock` in `tools/mcp_oauth_manager.py` with a binary semaphore released around the in-flight resource request, so a session-long streamable-HTTP GET no longer holds the auth lock and cross-task teardown cannot strand it — upstream's words: "structurally impossible now". It closed [#81051](https://github.com/NousResearch/hermes-agent/issues/81051); #77765 describes the same symptom and is plausibly also cured but is still open, so do not treat #77765's closure as the gate. **Do NOT chase this by rebasing onto `main`:** `9a1eef7a` is **311 commits ahead of v2026.8.27**, the newest release, and this fork is based on v2026.8.16 — that is 311 commits of untested-for-us drift to obtain one fix whose symptom is already mitigated. **Instead: at the next upstream RELEASE containing `9a1eef7a`, rebase this fork onto it** (the established cadence — the fork was rebased onto v2026.8.16 on 2026-08-16) and rerun the Validation suite below. Then re-evaluate BOTH the homelab-mcp startup health gate AND `hermes-agent-mcp-recover` (see Workaround), since both exist only because of this fault. |
| **Upstream** | [NousResearch/hermes-agent#53696](https://github.com/NousResearch/hermes-agent/pull/53696), [NousResearch/hermes-agent#72689](https://github.com/NousResearch/hermes-agent/pull/72689), [NousResearch/hermes-agent#88131](https://github.com/NousResearch/hermes-agent/pull/88131) (OAuth listener fix, submitted 2026-08-17), [NousResearch/hermes-agent#77765](https://github.com/NousResearch/hermes-agent/issues/77765) (long-lived OAuth HTTP MCP reconnect wedge, still open), [NousResearch/hermes-agent#97458](https://github.com/NousResearch/hermes-agent/pull/97458) (**the fix**, merged to main 2026-08-29, unreleased), [NousResearch/hermes-agent#81051](https://github.com/NousResearch/hermes-agent/issues/81051) (closed by it; carries our stack trace and confirms the fault on `mcp` 1.28.1 against a self-hosted provider, i.e. not Composio-specific), [modelcontextprotocol/python-sdk#3382](https://github.com/modelcontextprotocol/python-sdk/issues/3382) + [PR #2858](https://github.com/modelcontextprotocol/python-sdk/pull/2858) (the SDK-side fix, still open — SDK `main` still holds the lock across both yields, so **bumping the `mcp` pin does not help**) |
| **Impact** | Without the pin, Hermes cannot connect to forge's Signal transport; fresh CI runners/cold Forge stores can fail before building; and pasted OAuth callbacks cannot complete reliably with a fixed redirect port. Without the startup health gate, a deployment-time homelab-mcp outage can leave finance tools unavailable until Hermes is restarted. **Measured 2026-08-29: this cost a FOUR-DAY silent outage of the daily finance sentinel** (2026-08-27 06:22 → 08-29 09:06). The poisoned lock makes every reconnect fail with `TimeoutError` for the life of the process while hermes retries every ~9 minutes forever, so uptime and unit state both look healthy. Notably nothing in the alerting was broken — the heartbeat detected it every 6h and six pages reached Pushover with HTTP 200 — the outage persisted because six identical pages did not convert into someone running one restart. That is why the mitigation is `hermes-agent-mcp-recover` (auto-restart) rather than another alarm. Until REST recipient resolution is fixed, manual production cron runs compute their result but fail group delivery; normal live-gateway cron delivery remains the supported path. |
...
|-------|-------|
| **Added** | 2026-02-11 |
| **Last reviewed** | 2026-09-14 (compatibility refresh for exact upstream `29ab812ccb6c8a86b8147bae1bd3585d26372c4c`; source and Cargo dependency pins unchanged) |
...
| **Reason** | At `29ab812`, upstream `Crawler` (`src/crawler/mod.rs`) still does **not** hold a `SearchIndex` reference or write to Tantivy after an RSS crawl. RSS recipes remain in SQLite without becoming visible to ordinary search. The upstream schema also still defines `servings` and `total_time` as `FAST | STORED` without `INDEXED`, so numeric range queries need the local flags. |
| **Workaround** | Retains the optional shared index on `Crawler`, `(ProcessResult, recipe_id)` handling, new/updated RSS indexing and `mark_recipe_indexed`, GitHub indexed timestamps, numeric `INDEXED` flags, and startup database rebuild. It also retains field-space normalization, empty results for invalid queries, whitespace-only all-query behavior and offsets based on the clamped limit. Upstream's stemming, ranking, conjunctive queries, locale filters, full-count pagination and stale-schema refusal are unchanged. |
| **Proven redundancy** | `29ab812` already routes GitHub writes through `SearchIndex::commit()`, which commits and synchronously calls `reader.reload()`. The refresh keeps that upstream call, removes only the extra local reload/helper, and uses the same tested commit helper for RSS writes and database rebuilds. |
| **Regression coverage** | Existing `indexer::` and `crawler::tests::` Rust modules plus a SQLite-memory/temporary-index fixture cover new/update visibility, indexed timestamps, spaced field/range queries, invalid/whitespace queries, clamped pagination and removal of stale documents during rebuild. The fixture performs no HTTP requests or live service operations. |
| **Validation** | Patch application and a complete Darwin package build passed against the verified source NAR `sha256-n7kNgUGRy9vvoS9lQKrb96Yjib3VeuuKqZwXbqly1e4=`. The existing Rust check hook, enabled only for this validation with `--locked --lib -- indexer:: crawler::tests::`, passed 51 tests (60 unrelated tests filtered out). The existing Cargo vendor hash validated unchanged; Forge/setup preservation checks also passed. Required public CI remains a separate promotion gate. |
| **Check** | At each nvfetcher bump, compare each hunk against the exact pinned source and remove only behaviorally equivalent redundancy. Do not discard the whole workaround merely because one upstream API changes. Keep the source/Cargo pins and separate CSS workaround unchanged unless a separately demonstrated compatibility issue requires otherwise. |
...
| **Managed webserver** | The pre-start script pins MA's advertised Base URL to `http://10.20.0.30:8095` and its bind port to `8095`. The address is Forge's known primary LAN IP (the interface uses DHCP); update it if that address changes. The human-facing `https://music.holthome.net` remains on Caddy with PocketID protection. Setting the internal port to the external TLS port `443` makes the webserver fail with `PermissionError`, producing Caddy 502 responses. |
| **HA reconnect fix (2026-09-13)** | The previous pre-start argument overwrote the working internal Base URL with the public HTTPS URL on every MA restart. HA then attempted `wss://music.holthome.net/ws`, where PocketID redirected it to browser login, leaving `media_player.patio_mass` and `media_player.pavilion_mass` unavailable. Managing the internal URL durably prevents this overwrite without changing Spotify token migration, MA authentication, Caddy protection, or firewall exposure. |
| **Post-deployment acceptance** | Deployment requires confirmation. Update HA's existing Music Assistant entry to `http://10.20.0.30:8095` while retaining its token; do not delete/recreate the entry. Restart Music Assistant, confirm `/info` advertises the reachable internal URL and HA still uses it, both outdoor `*_mass` entities remain available, Spotify and Sonos providers load, and the public Caddy UI still requires PocketID. Repeat the restart to check persistence. |
